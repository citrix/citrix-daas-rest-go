...
type CatalogCMD interface {
	...
	*/
	...
	/*
		GetCustomerCatalogsStatus Returns the Catalogs that the specified customer has created in the specified site

		@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
		@param customerId Specific customer id have catalogs retrieved for
		@param siteId
		@return ApiGetCustomerCatalogsStatusRequest
	*/
	GetCustomerCatalogsStatus(ctx context.Context, customerId string, siteId string) ApiGetCustomerCatalogsStatusRequest

	// GetCustomerCatalogsStatusExecute executes the request
	//  @return CustomerCatalogsStatusOverview
	GetCustomerCatalogsStatusExecute(r ApiGetCustomerCatalogsStatusRequest) (*CustomerCatalogsStatusOverview, *http.Response, error)
...
type ApiGetCustomerCatalogsStatusRequest struct {
	ctx                 context.Context
	ApiService          CatalogCMD
	customerId          string
	siteId              string
...
// The catalog ids for which to get the status
func (r ApiGetCustomerCatalogsStatusRequest) CatalogId(catalogId []string) ApiGetCustomerCatalogsStatusRequest {
	r.catalogId = &catalogId
	return r
}

// The Transaction Id.
func (r ApiGetCustomerCatalogsStatusRequest) CitrixTransactionId(citrixTransactionId string) ApiGetCustomerCatalogsStatusRequest {
	r.citrixTransactionId = &citrixTransactionId
	return r
}

func (r ApiGetCustomerCatalogsStatusRequest) Execute() (*CustomerCatalogsStatusOverview, *http.Response, error) {
	return r.ApiService.GetCustomerCatalogsStatusExecute(r)
}
...
GetCustomerCatalogsStatus Returns the Catalogs that the specified customer has created in the specified site
	...
	@return ApiGetCustomerCatalogsStatusRequest
*/
func (a *CatalogCMDService) GetCustomerCatalogsStatus(ctx context.Context, customerId string, siteId string) ApiGetCustomerCatalogsStatusRequest {
	return ApiGetCustomerCatalogsStatusRequest{
		ApiService: a,
		ctx:        ctx,
		customerId: customerId,
...
//
//	@return CustomerCatalogsStatusOverview
func (a *CatalogCMDService) GetCustomerCatalogsStatusExecute(r ApiGetCustomerCatalogsStatusRequest) (*CustomerCatalogsStatusOverview, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *CustomerCatalogsStatusOverview
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "CatalogCMDService.GetCustomerCatalogsStatus")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/{customerId}/{siteId}/catalogs/status"
	localVarPath = strings.Replace(localVarPath, "{"+"customerId"+"}", url.PathEscape(parameterValueToString(r.customerId, "customerId")), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"siteId"+"}", url.PathEscape(parameterValueToString(r.siteId, "siteId")), -1)
	...
	if r.catalogId != nil {
		...
		if reflect.TypeOf(t).Kind() == reflect.Slice {
			s := reflect.ValueOf(t)
			for i := 0; i < s.Len(); i++ {
				parameterAddToHeaderOrQuery(localVarQueryParams, "catalogId", s.Index(i).Interface(), "multi")
			}
		} else {
			parameterAddToHeaderOrQuery(localVarQueryParams, "catalogId", t, "multi")
		}
	}
	// to determine the Content-Type header
	localVarHTTPContentTypes := []string{}
